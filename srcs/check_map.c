...
static void	check_border(t_game *gamyun)
{
	...
	if (gamyun->height >= 50)
		exit_error(gamyun, "Geçersiz Harita: Haritanın yüksekliği çok fazla.\n\
Invalid Map: The height of the map is too high", 0);
	if (gamyun->width >= 50)
		exit_error(gamyun, "Geçersiz Harita: Haritanın genişliği çok fazla.\n\
...
static void	check_char(t_game *gamyun)
{
	...
	if (!gamyun->c)
		exit_error(gamyun, "Geçersiz harita: Toplanacak öge yok.\n\
Invalid map: No items to collect.", 0);
	if (gamyun->e != 1)
		exit_error(gamyun, "Geçersiz harita: Çıkış yok yada birden fazla.\n\
Invalid map: No exit or more than one.", 0);
	if (gamyun->p != 1)
		exit_error(gamyun, "Geçersiz harita: Oyuncu yok yada birden fazla.\n\
